...
import { 
  ...
  User, Calendar, CreditCard, ArrowRight, Printer, X, FileText, Trash2
} from "lucide-react";
...
export default function OrderManagement() {
  const { orders, updateOrderStatus } = useIn();
  const [filter, setFilter] = useState("All");
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedOrder, setSelectedOrder] = useState(null); // For Invoice Popup
  ...
  const handleBulkShip = () => {
    if(selectedOrders.length === 0) return alert("Please select orders first!");
    alert(`Generating Shipping Labels for ${selectedOrders.length} orders...`);
  };

  const handleDeleteOrder = (id) => {
  if (!window.confirm("Are you sure you want to delete this order?")) return;

  // Temporary delete (later replace with Firebase delete)
  const updatedOrders = orders.filter(order => order.id !== id);
  updateOrderStatus(updatedOrders); // if context supports full update
};
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-slate-50/50 text-[10px] font-black text-slate-400 uppercase tracking-widest">
            <tr>
              <th className="p-6 w-10">Select</th>
          ...
          <tbody className="divide-y divide-slate-50">
            {filteredOrders.map((order) => (
              <tr key={order.id} className={`hover:bg-slate-50/30 transition-colors ${selectedOrders.includes(order.id) ? 'bg-blue-50/50' : ''}`}>
                ...
                <td className="p-6 font-bold text-slate-700 text-sm uppercase">{order.customer}</td>
                <td className="p-6 font-black text-slate-900">₹{order.total}</td>
                <td className="p-6">
                  <select
  value={order.status}
  onChange={(e) => updateOrderStatus(order.id, e.target.value)}
  className={`text-[9px] font-black px-4 py-2 rounded-full border-none outline-none cursor-pointer transition-all
    ...
    <Trash2 size={14} />
      ...
      {selectedOrder && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden relative animate-in fade-in zoom-in duration-200">
            <button onClick={() => setSelectedOrder(null)} className="absolute top-8 right-8 text-slate-400 hover:text-red-500 transition-colors">
              <X size={24} />
            </button>
            
            <div className="p-12 space-y-8" id="invoice-print">
              <div className="flex justify-between items-start border-b border-slate-100 pb-8">
                ...
                <div className="text-right">
                  ...
                  <p className="font-black text-slate-900 uppercase">{selectedOrder.id}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-8">
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-bold">Tamil Nadu, India</p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] font-black text-slate-400 uppercase mb-1">Status</p>
                  <span className="text-[10px] font-black bg-blue-100 text-blue-600 px-3 py-1 rounded-full uppercase italic">
              ...
              <div className="flex justify-between items-center border-t border-slate-100 pt-8">
                <p className="text-xs text-slate-400 font-bold uppercase">Paid via Online</p>
                <div className="text-right">
              ...
              <div className="flex gap-4 no-print mt-4">
                ...
                <button className="flex-1 bg-blue-600 text-white py-4 rounded-2xl font-black text-xs uppercase flex items-center justify-center gap-2 shadow-lg shadow-blue-100">
