...
const ProductListing = () => {
  ...
  const filteredProducts = useMemo(() => {
    let data = products.filter((p) => {
      ...
      const matchGreen = !greenOnly || p.isGreenCracker;
  ...
  // INSIDE THE RETURN MAP:
{filteredProducts.map((p) => {
  const isAvailable = Number(p.stockQty) > 0 && !p.isOutOfStock;

  return (
    <div key={p.id} className="bg-white rounded-[2.5rem] border border-slate-100 overflow-hidden hover:shadow-2xl transition-all group">
      <div className="h-64 relative overflow-hidden bg-slate-50">
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-2">
          {p.isGreen && (
            <div className="bg-emerald-500 text-white p-2 rounded-xl shadow-lg border-2 border-white">
              <Leaf size={16} />
            </div>
          )}
      ...
      <div className="p-6">
        <div className="flex justify-between items-start mb-2">
          ...
          <span className="text-[10px] font-bold text-slate-400 flex items-center gap-1">
            <Flame size={12} className="text-orange-400" /> {p.noiseLevel || 'Medium Noise'}
          </span>
        </div>
        <h3 className="text-xl font-black text-slate-900 mb-4 tracking-tight leading-tight">{p.name}</h3>

        {/* ... rest of the card (Price and ShoppingCart button) */}
      </div>
    </div>
  );
})}
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-10">
      {/* HEADER - Updated to match Logo Colors */}
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 flex flex-col lg:flex-row gap-8">
        ...
        <aside className="w-full lg:w-72">
          <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100 sticky top-28 space-y-8">
            ...
            <div className="space-y-3">
              ...
              <select onChange={(e) => setDuration(Number(e.target.value))} className="w-full p-3 rounded-xl bg-slate-50 border-none font-bold text-sm text-slate-700">
        ...
        <main className="flex-1">
          ...
          <div className={view === "grid" ? "grid sm:grid-cols-2 xl:grid-cols-3 gap-8" : "space-y-4"}>
            {filteredProducts.map((p) => {
              // LOGIC: Check Stock quantity instead of a boolean field
              const isAvailable = Number(p.stockQty) > 0;
              ...
              return (
                <div key={p.id} className="bg-white rounded-[2.5rem] border border-slate-100 overflow-hidden hover:shadow-2xl hover:-translate-y-1 transition-all group">
                  <div className="h-64 relative overflow-hidden bg-slate-50">
                    ...
                    {p.isGreenCracker && (
                      <div className="absolute top-4 left-4 bg-emerald-500 text-white p-2 rounded-xl shadow-lg border-2 border-white">
                        <Leaf size={16} />
                      </div>
                    )}
                    
                    {/* Visual Stock Warning only if truly 0 */}
                    {!isAvailable && (
                      <div className="absolute inset-0 bg-white/60 backdrop-blur-[2px] flex items-center justify-center">
                        <span className="bg-red-600 text-white text-[10px] font-black px-4 py-2 rounded-full uppercase tracking-tighter">Temporarily Sold Out</span>
                      </div>
