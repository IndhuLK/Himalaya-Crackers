...
import {
  X, ShoppingCart, Rocket, Trash2, Plus, Minus,
  User, Mail, PhoneCall, MapPin, Building, Hash,
} from "lucide-react";
  ...
  return (
    <div className="fixed inset-0 z-[120] bg-black/70 backdrop-blur-sm flex justify-end">
      <div className="w-full max-w-6xl bg-white h-full flex flex-col md:flex-row shadow-2xl animate-in slide-in-from-right duration-300">
        ...
        {/* --- LEFT: CART ITEMS --- */}
        <div className={`flex-1 flex flex-col border-r bg-slate-50 ${step === "checkout" ? "hidden md:flex" : "flex"}`}>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {cartItems.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-slate-300 py-20">
                ...
                <p className="font-black text-xl mt-4 italic uppercase tracking-widest">Basket is Empty</p>
              </div>
            ) : (
              cartItems.map((item) => (
                <div key={item.id} className="flex items-center gap-4 bg-white p-4 rounded-[2rem] border border-slate-100 shadow-sm transition-hover hover:shadow-md">
                  <div className="bg-blue-50 p-4 rounded-2xl shrink-0"><Rocket className="text-blue-600" /></div>
          ...
          <div className="bg-white border-t p-8 space-y-4">
            ...
            <button
              ...
              className="md:hidden w-full bg-slate-900 text-white py-5 rounded-[2rem] font-black uppercase tracking-widest shadow-xl disabled:bg-slate-200"
            >
        ...
        {/* --- RIGHT: SHIPPING FORM --- */}
        <div className={`w-full md:w-[450px] flex flex-col bg-white ${step === "cart" ? "hidden md:flex" : "flex"}`}>
          <div className="bg-slate-900 text-white p-6 flex justify-between items-center">
            <h3 className="font-black uppercase tracking-widest italic">Shipping Details</h3>
            <button onClick={() => setStep("cart")} className="md:hidden"><X /></button>
          </div>

          <div className="p-8 space-y-6 overflow-y-auto">
            <Input label="Full Name *" icon={User}>
              <input name="name" placeholder="Receiver Name" onChange={(e) => setCustomer({ ...customer, name: e.target.value })} />
            </Input>
            <Input label="WhatsApp Number *" icon={PhoneCall}>
              <input name="mobile" type="tel" placeholder="9123456789" onChange={(e) => setCustomer({ ...customer, mobile: e.target.value })} />
            </Input>
            <Input label="Delivery Address *" icon={MapPin}>
              <textarea name="address" rows="2" placeholder="Complete Address" onChange={(e) => setCustomer({ ...customer, address: e.target.value })} />
            </Input>
            <div className="grid grid-cols-2 gap-4">
              <Input label="City" icon={Building}>
                <input name="city" placeholder="City" onChange={(e) => setCustomer({ ...customer, city: e.target.value })} />
              </Input>
              <Input label="Pincode *" icon={Hash}>
                <input name="pincode" type="number" placeholder="600001" onChange={(e) => setCustomer({ ...customer, pincode: e.target.value })} />
              </Input>
            </div>
            ...
            <button
              ...
              className="w-full mt-6 bg-[#F2A31E] hover:bg-orange-600 text-white py-5 rounded-[2rem] font-black uppercase tracking-widest shadow-2xl transition-all active:scale-95 disabled:bg-slate-200"
            >
...
const Input = ({ label, icon: Icon, children }) => (
  <div className="space-y-2">
    ...
    {React.cloneElement(children, {
      className: "w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 font-bold outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all text-sm",
    })}
  </div>
);
